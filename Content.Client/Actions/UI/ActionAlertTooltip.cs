    ...
    public sealed class ActionAlertTooltip : PanelContainer
    {
        ...
        protected override void FrameUpdate(FrameEventArgs args)
        {
            ...
            if (timeLeft > TimeSpan.Zero)
            {
                ...
                if (!FormattedMessage.TryFromMarkup($"[color=#a10505]{(int) duration.TotalSeconds} сек задержки ({(int) timeLeft.TotalSeconds + 1} сек восстоновления)[/color]", out var markup))
                    return;
